...
class SmoothEAR:
    ...
    baseline:  float      # rolling mean of ema_avg (open-eye reference)
    deviation: float      # baseline − ema_avg  (positive → eyes below baseline)
...
class EarSmoother:
    ...
    def __init__(self, alpha: float = 0.15, window_seconds: float = 4.0):
        ...
        # Rolling window: (timestamp_ms, ema_avg)
    ...
    def update(
        ...
        if self._ema_left is None:
            # Bootstrap: first frame is exact (no warmup lag)
        ...
        deviation = baseline - ema_avg   # positive → eyes are below baseline
