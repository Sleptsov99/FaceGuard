...
class TestEarSmootherEMA(unittest.TestCase):
    ...
    def test_single_noise_spike_barely_moves_ema(self):
        ...
        """
        # Warm up to a stable baseline of 0.30
        for i in range(30):
            self.s.update(0.30, 0.30, _ts(i * 33))

        # Single spike frame
        out = self.s.update(0.05, 0.05, _ts(30 * 33))
        # EMA ≈ 0.15*0.05 + 0.85*0.30 = 0.2625 — well above 0.20
    ...
    def test_ema_converges_to_new_level(self):
        ...
        # 60 frames at 0.10 (genuine closure)
        for i in range(60):
            out = self.s.update(0.10, 0.10, _ts((30 + i) * 33))

        self.assertLess(out.ema_avg, 0.20)  # must have converged below threshold
...
class TestEarSmootherBaseline(unittest.TestCase):
    def test_baseline_equals_mean_of_window(self):
        ...
        s = EarSmoother(alpha=1.0, window_seconds=10.0)  # alpha=1 → EMA == raw
        for i in range(50):
    ...
    def test_deviation_positive_when_below_baseline(self):
        ...
        # baseline ≈ 0.30, ema_avg ≈ 0.10 → deviation > 0
    ...
    def test_old_frames_pruned_from_window(self):
        s = EarSmoother(alpha=1.0, window_seconds=1.0)  # 1 second window
        # Feed 30 frames at 0.30 spread over 2 seconds (outside window)
        for i in range(30):
            s.update(0.30, 0.30, _ts(i * 66))
        # Now feed frames at 0.10 — baseline should drift toward 0.10
        for i in range(30):
            out = s.update(0.10, 0.10, _ts(2_000 + i * 33))
        # Old 0.30 frames pruned, baseline should be near 0.10
...
class TestBlinkDebounce(unittest.TestCase):
    def test_min_frames_closed_1_triggers_immediately(self):
        ...
        det.update(0.10, _ts(0))      # closed: blink starts
    ...
    def test_min_frames_closed_2_requires_two_consecutive_frames(self):
        ...
        det.update(0.10, _ts(0))           # 1st closed frame  — not enough
        event = det.update(0.30, _ts(100)) # eye opens before reaching 2 frames
        self.assertIsNone(event)           # no blink recorded
    ...
    def test_min_frames_closed_2_succeeds_with_two_consecutive(self):
        ...
        det.update(0.10, _ts(0))           # 1st closed
        det.update(0.10, _ts(33))          # 2nd closed → blink starts
        event = det.update(0.30, _ts(100))
        self.assertIsNotNone(event)
    ...
    def test_streak_resets_on_interruption(self):
        ...
        det.update(0.10, _ts(0))    # streak = 1
        det.update(0.30, _ts(33))   # open → streak reset
        det.update(0.10, _ts(66))   # streak = 1 again (not 2)
        det.update(0.10, _ts(99))   # streak = 2 (still not 3)
        event = det.update(0.30, _ts(132))
        self.assertIsNone(event)   # streak never reached 3
    ...
    def test_min_frames_open_2_delays_blink_end(self):
        ...
        det.update(0.10, _ts(0))           # blink starts
        event1 = det.update(0.30, _ts(100))  # 1st open frame — not enough
        self.assertIsNone(event1)
        event2 = det.update(0.30, _ts(133))  # 2nd open frame → blink ends
    ...
    def test_blink_start_timestamp_is_run_start_not_debounce_end(self):
        ...
        """
        det = BlinkDetector(min_frames_closed=2, min_blink_duration_ms=0)
        det.update(0.10, _ts(0))    # first closed frame, ts=1_000_000
        det.update(0.10, _ts(33))   # second: blink committed, start = _ts(0)
    ...
    def test_blink_end_timestamp_is_first_open_frame(self):
        ...
        det.update(0.30, _ts(100))  # 1st open frame — run start
        event = det.update(0.30, _ts(133))  # 2nd open frame — blink committed
        self.assertIsNotNone(event)
    ...
    def test_ema_prevents_spike_from_reaching_detector(self):
        ...
        # Spike is absorbed by EMA → no blink triggered
