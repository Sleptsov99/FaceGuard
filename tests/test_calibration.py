...
class TestCalibrationLifecycle(unittest.TestCase):
    ...
    def test_restart_clears_previous_samples(self):
        ...
        s.start(_ts(0))
        # Feed some frames with EAR=0.10 (closed eye — not collected for baseline)
        for i in range(10):
            s.update(_ok_detection(), _metrics(ear=0.10, state=EyeState.CLOSED), _ts(i * 33))
        # Restart
        s.start(_ts(500))
        # Feed frames with EAR=0.30 (open)
        for i in range(100):
        ...
        if s.state == CalibrationState.DONE:
            profile = s.finish()
            # EAR should be near 0.30, not contaminated by 0.10 from first run
...
class TestCalibrationCompletion(unittest.TestCase):
    ...
    def test_finish_is_idempotent(self):
        ...
        p1 = s.finish()
        p2 = s.finish()
        self.assertIs(p1, p2)   # same object, no recomputation


# ─── CalibrationProfile thresholds ───────────────────────────────────────────
...
class TestProfileThresholds(unittest.TestCase):
    ...
    def test_ear_thresholds_within_physiological_bounds(self):
        # Even with extreme std, thresholds are clamped
    ...
    def test_yaw_threshold_accounts_for_personal_mean(self):
        ...
        # Both should have same threshold value (threshold is deviation, not absolute)
    ...
    def test_blink_duration_threshold_from_samples(self):
        ...
        # mean ≈ 130, std ≈ 22, long = 130 + 2*22 ≈ 174 → clamped to 300
    ...
    def test_no_blinks_gives_default_duration(self):
        ...
        # Should fall back to global default (150 ms mean + 2*50 std = 250 → clamped 300)
        self.assertGreaterEqual(p.long_blink_threshold_ms, 300.0)
...
class TestApplyCalibration(unittest.TestCase):
    ...
    def test_distraction_uses_personal_mean_for_deviation(self):
        ...
        dist.apply_calibration(self._make_profile())   # yaw_mean=5, threshold=25
        # yaw=20° → deviation from mean = 15° < threshold 25° → not looking away
        det = DetectionResult(
